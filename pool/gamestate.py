...
class GameState:
    ...
    def create_white_ball(self):
        ...
        while check_if_ball_touches_balls(ball_pos, 0, self.balls):
            ball_pos = [
                random.randint(
                    int(config.table_margin + config.ball_radius + config.hole_radius),
    ...
    def set_pool_balls(self):
        ...
        coord_shift = np.array(
            [math.sin(math.radians(60)) * config.ball_radius * 2, -config.ball_radius]
        )
    ...
    def redraw_all(self, update=True):
        ...
        if not self.all_not_moving():
            print(f"Turn number: {self.turn_number}\n\n")

            print(json.dumps(self.white_ball.convert_to_dict()))
            for b in self.balls:
                print(type(b))
                print(json.dumps(b.convert_to_dict()))
        ...
        for num, point in enumerate(table_side_points[:-1]):
            ...
            if num % 4 != 1:
                self.table_sides.append(
                    table_sprites.TableSide([point, table_side_points[num + 1]])
                )
        self.table_sides.append(
            table_sprites.TableSide([table_side_points[-1], table_side_points[0]])
        )
    ...
    def game_over(self, p1_won):
        ...
        self.canvas.surface.blit(
            rendered_text, (config.resolution - font.size(text)) / 2
        )
        pygame.display.flip()
        pygame.event.clear()
        paused = True
    ...
    def check_remaining(self):
        ...
        for remaining_ball in self.balls:
            if remaining_ball.number != 0 and remaining_ball.number != 8:
                stripes_remaining = (
                    stripes_remaining or remaining_ball.ball_type == BallType.Striped
                )
                solids_remaining = (
                    solids_remaining or not remaining_ball.ball_type == BallType.Striped
                )
        ball_type_remaining = {
            BallType.Solid: solids_remaining,
            BallType.Striped: stripes_remaining,
        }

        # decides if on of the players (or both) should be potting 8ball
        self.potting_8ball = {
            Player.Player1: not ball_type_remaining[
                self.ball_assignment[Player.Player1]
            ],
            Player.Player2: not ball_type_remaining[
                self.ball_assignment[Player.Player2]
            ],
        }

    def first_collision(self, ball_combination):
        self.white_ball_1st_hit_is_set = True
        self.white_ball_1st_hit_8ball = (
            ball_combination[0].number == 8 or ball_combination[1].number == 8
        )
    ...
    def potted_ball_rules(self):
        if len(self.potted) > 0:
            ...
            if only_solids_potted or only_stripes_potted:
                selected_ball_type = (
                    BallType.Striped if only_stripes_potted else BallType.Solid
                )
                if self.ball_assignment is None:
                    ...
                    self.ball_assignment = {
                        self.current_player: selected_ball_type,
                        other_player: other_ball_type,
                    }
                    self.potting_8ball = {
    ...
    def first_hit_rule(self):
        ...
        if not self.white_ball_1st_hit_is_set:
            ...
            if (
                not self.white_ball_1st_hit_8ball
                and self.ball_assignment[self.current_player]
                != self.white_ball_1st_hit_type
            ):
                self.turn_over(True)
